...
final class CodexStatusTests: XCTestCase {
  func testPhysicalLiveSnapshotPersistsProtectedAppGroupCache() async throws {
    guard ProcessInfo.processInfo.environment["CODEX_STATUS_PHYSICAL_LIVE"] == "1" else {
      throw XCTSkip("Physical App Attest integration proof is opt-in.")
    }
    ...
    do {
      ...
      XCTFail(Self.safePhysicalFailure(stage: "fetch", error: error))
      return
    }

    XCTAssertEqual(snapshot.schemaVersion, 1)
    XCTAssertGreaterThan(snapshot.summary.configuredAccounts, 0)
    XCTAssertGreaterThanOrEqual(snapshot.summary.enabledAccounts, snapshot.summary.usableNow)

    do {
      try SnapshotCache.save(snapshot)
    } catch {
      XCTFail(Self.safePhysicalFailure(stage: "cache_write", error: error))
      return
    }

    guard let loaded = SnapshotCache.load() else {
      XCTFail("physical_live_stage=cache_reload classification=missing_snapshot")
      return
    }
    XCTAssertEqual(loaded, snapshot)

    let encoded = try SnapshotCache.encoded(loaded)
  ...
  func testPhysicalAppLaunchSnapshotIsAvailableToExtensions() throws {
    guard ProcessInfo.processInfo.environment["CODEX_STATUS_PHYSICAL_LIVE"] == "1" else {
      throw XCTSkip("Physical App Group integration proof is opt-in.")
    }

    let snapshot = try XCTUnwrap(
      SnapshotCache.load(),
      "physical_live_stage=extension_cache classification=missing_snapshot"
    )
    XCTAssertEqual(snapshot.schemaVersion, 1)
    XCTAssertGreaterThan(snapshot.summary.configuredAccounts, 0)
    XCTAssertGreaterThanOrEqual(snapshot.summary.enabledAccounts, snapshot.summary.usableNow)
  ...
  private static func safePhysicalFailure(stage: String, error: Error) -> String {
    if let clientError = error as? CapacityClientError {
      let classification: String
      switch clientError {
      case .appAttestUnavailable:
        classification = "app_attest_unavailable"
      case .invalidServerResponse:
        classification = "invalid_server_response"
      case .serverRejected(let status, _):
        classification = "server_rejected_\(status)"
      case .challengeInvalid:
        classification = "challenge_invalid"
      }
      return "physical_live_stage=\(stage) classification=\(classification)"
    }
    if let cacheError = error as? SnapshotCacheError {
      let classification: String
      switch cacheError {
      case .appGroupUnavailable:
        classification = "app_group_unavailable"
      case .encodingFailed:
        classification = "encoding_failed"
      #if DEBUG
        case .diagnosticPayloadMissing:
          classification = "diagnostic_payload_missing"
        case .diagnosticPayloadInvalid:
          classification = "diagnostic_payload_invalid"
      #endif
      }
      return "physical_live_stage=\(stage) classification=\(classification)"
    }
    let cocoaError = error as NSError
    return "physical_live_stage=\(stage) classification=\(cocoaError.domain)_\(cocoaError.code)"
  }
}
