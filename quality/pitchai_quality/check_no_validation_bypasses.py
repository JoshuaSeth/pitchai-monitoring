...
_EXPECTED_PORTABLE_MANIFEST_SHA256 = "5b584e8a5333ee78137c4d2045237d811dbebbd6ec5b9fceb5687a7f7131e412"
_CONFIG_PATHS = (
...
def _strict_workflow_violations() -> list[Violation]:
    ...
    for command in required_commands:
        if stripped_lines.count(command) != 1:
            ...
            break
    digest_matches = {match.group("kind"): match for match in TRUST_DIGEST_ASSIGNMENT.finditer(workflow_text)}
    if set(digest_matches) != {"baseline", "verifier"}:
        violations.append(Violation(_STRICT_WORKFLOW_PATH, 1, "strict workflow trust anchors are missing"))
        return violations
    match = digest_matches["verifier"]
    ...
    normalized_workflow = TRUST_DIGEST_ASSIGNMENT.sub(
        lambda digest_match: (
            f'{digest_match.group("prefix")}<{digest_match.group("kind").upper()}_SHA256>'
            f'{digest_match.group("suffix")}'
        ),
        workflow_text,
    )
