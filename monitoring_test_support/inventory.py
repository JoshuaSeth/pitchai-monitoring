...
from monitoring_contracts.json_types import json_object, object_list, text_value
from monitoring_contracts.legacy import load_config
...
def production_config() -> JsonObject:
    ...
    return json_object(cast("JsonInput", load_config(CONFIG_PATH)))


def production_domains() -> list[JsonObject]:
    """Return every active production monitoring entry."""
    return object_list(production_config().get("domains"))
