...
def load_domain_spec_from_module_dict(module_vars: JsonObject) -> DomainCheckSpec:
    ...
    """
    return common_runtime.load_domain_spec_from_module_dict(module_vars)
...
def load_config(path: Path) -> JsonObject:
    ...
    """
    return json_object(cast("JsonInput", monitoring_runtime.load_config(path)))
...
def load_domain_spec(entry: JsonObject) -> DomainCheckSpec:
    ...
    """
    return monitoring_runtime.load_domain_spec(entry)
...
def domain_alert_policy(*, telegram: str, reason: str | None = None) -> AlertPolicy:
    ...
    """
    return inventory_runtime.DomainAlertPolicy(telegram, reason)
...
def domain_entry_config(
    *,
    ...
    """
    return monitoring_runtime.DomainEntryConfig(domain, raw_entry, alert_policy)
