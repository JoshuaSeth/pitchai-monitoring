...
async def test_domain_telegram_router_suppresses_dashboard_only_and_routes_critical(
    ...
    """Route only critical production domains to Telegram."""
    sent: list[str] = []

    async def fake_send(
        _client: httpx.AsyncClient,
        _telegram_cfg: TelegramConfig,
        message: str,
    ...
    for domain in sorted(EXPECTED_DASHBOARD_ONLY_DOMAINS):
        ...
        if routed is not None:
            pytest.fail(f"dashboard-only domain routed to Telegram: {domain}")

    if sent:
        pytest.fail(f"dashboard-only domains emitted Telegram messages: {sent!r}")
    ...
    routed = await route_domain_telegram_alert(
        http_client=http_client,
        telegram_cfg=telegram_cfg,
        entry=critical_entry,
        message="down: pitchai.net",
    )
    if routed != (True, [{"ok": True}]):
        pytest.fail(f"critical domain returned the wrong Telegram result: {routed!r}")
    if sent != ["down: pitchai.net"]:
        pytest.fail(f"critical domain did not emit exactly one Telegram message: {sent!r}")
...
async def test_every_inventory_domain_enters_http_and_browser_check_pipeline(
    ...
    specs = [load_domain_spec(entry) for entry in production_domains()]
    ...
    if frozenset(result.domain for result in results) != EXPECTED_ACTIVE_DOMAINS:
    ...
    failed_results = [result.domain for result in results if not result.ok]
    if failed_results:
