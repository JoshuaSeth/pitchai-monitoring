...
`e2e_registry/hotpath_inventory.json` is the checked-in runtime inventory. Version 1 contains the active 15-lane estate: DFT, AutoPAR, two intentional AIPC/SkyBuyFly lanes, potAIto, AFASAsk/GZB, Orthoparse, DePlanBook Play, CISNL, AIGENDA Rules, DePlanBook CMS, Apologetica CMS, AIGENDA Calendar, the PitchAI public website, and the QuickChat Waddinxveen demo. OrthoShare is not admitted because it has no deployed product surface. The QuickChat RSR lane retired on 2026-09-21 with the RSR STUDENT CHATBOT demo teardown; its reminder is stopped and it must not be re-admitted. The Waddinxveen demo lane is a separate identity that owns the only synthetic question on that municipality demo surface, because the reviewed availability check for `waddinxveen.demos.pitchai.net` stays read-only and a green host check cannot see a chatbot that stops answering.

The checked-in inventory is the single runtime source of truth. Deployment removes any `E2E_HOTPATH_INVENTORY_PATH` override from the registry environment and then asserts that the lane set the running registry actually serves equals the checked-in lane set, so a hand-installed copy cannot keep a retired lane admitted or hide a newly admitted one.
