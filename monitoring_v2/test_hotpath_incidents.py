...
from .hotpath_store_runtime import HOTPATH_CODEC, HOTPATH_READ, HOTPATH_SCHEMA, HOTPATH_WRITE
...
if TYPE_CHECKING:
    ...
    from .hotpath_store_runtime import IngestedReport
...
def _report(
    ...
    lane_id = _SYNTHETIC_LANE_ID if options.synthetic else lane.lane_id
    raw: JsonInput = {
        ...
        "lane_id": lane_id,
        "name": _SYNTHETIC_NAME if options.synthetic else lane.name,
        "occurred_at": datetime.fromtimestamp(occurred_at_ts, tz=UTC).isoformat(),
        "project": _SYNTHETIC_PROJECT if options.synthetic else lane.project,
        ...
        "target_surface": _SYNTHETIC_TARGET if options.synthetic else lane.target_surface,
    }
...
def _ingest(
    ...
    return HOTPATH_WRITE.ingest_report(
        str(db_path),
        inventory,
        report,
        lane,
...
def test_critical_transition_dedupes_changes_and_recovers(tmp_path: Path) -> None:
    ...
    with closing(HOTPATH_SCHEMA.connect(str(database))) as connection:
        ...
        details: tuple[JsonObject, ...] = (
            HOTPATH_CODEC.decode_object(HOTPATH_SCHEMA.row_string(rows[0], "details_json")),
            HOTPATH_CODEC.decode_object(HOTPATH_SCHEMA.row_string(rows[1], "details_json")),
            HOTPATH_CODEC.decode_object(HOTPATH_SCHEMA.row_string(rows[-1], "details_json")),
        )
...
def test_out_of_order_and_synthetic_reports_never_replace_real_lane_state(tmp_path: Path) -> None:
    ...
    old_result = _ingest(
        database,
        inventory,
        _report(lane, 1900, _ReportOptions(success=False)),
        2100,
    )
    pass_result = _ingest(
        database,
        inventory,
        _report(lane, 2200, _ReportOptions(success=True, synthetic=True)),
        2200,
    )
    fail_result = _ingest(
        database,
    ...
    snapshot = HOTPATH_READ.build_hotpath_snapshot(str(database), inventory, now_ts=2400)
    ...
    with closing(HOTPATH_SCHEMA.connect(str(database))) as connection:
        rows = _outbox(connection)
        details = HOTPATH_CODEC.decode_object(
            HOTPATH_SCHEMA.row_string(rows[0], "details_json"),
        )
