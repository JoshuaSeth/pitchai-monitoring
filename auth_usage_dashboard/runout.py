...
def select_capacity_basis(accounts: list[dict[str, Any]]) -> dict[str, Any]:
    eligible = [
        account
    ...
    counts = {
        key: sum(
            ...
            if account.get(key, {}).get("reported") is True
            and isinstance(account.get(key, {}).get("remaining_percent"), (int, float))
        )
        for key in ("five_hour", "weekly")
    }
    minimum_short_window_coverage = max(1, math.ceil(len(eligible) / 2))
    if counts["five_hour"] >= minimum_short_window_coverage:
        key: str | None = "five_hour"
    elif counts["weekly"]:
        key = "weekly"
    elif counts["five_hour"]:
        key = "five_hour"
    else:
        key = None
...
def build_runout_forecast(
    ...
    *,
    ...
    now: datetime,
    capacity_basis: dict[str, Any] | None = None,
) -> dict[str, Any]:
    now = now.astimezone(UTC)
    capacity_basis = capacity_basis or select_capacity_basis(accounts)
    ...
    measured_accounts = [
        ...
        and account.get("auth_valid") is True
        and not account.get("stale")
        and account.get(window_key, {}).get("reported") is True
        and isinstance(
            account.get(window_key, {}).get("remaining_percent"), (int, float)
        )
    ]
    ...
    initial, events = _capacity_schedule(
        accounts,
        now=now,
    ...
    weekly_blocked = sum(
        1 for account in accounts if account.get("status") == "weekly_limited"
    )
    near_weekly = sum(
        1
        for account in accounts
    ...
    drivers = [
        f"{base_rate:.1f} {_window_label(window_key).lower()} capacity points/hour at the current burn estimate",
    ...
    return {
        ...
        "methodology": {
            ...
            "scenario_count": len(rates),
            "automatic_resets_included": [window_key],
            "weekly_handling": "Weekly exhaustion blocks an account until reset. Weekly percentage points are used only when weekly is the declared forecast basis.",
...
def _unavailable_forecast(
    ...
    *,
    ...
    usable_now = sum(
        ...
        for account in accounts
        if account.get("enabled")
        and account.get("selectable_now")
        and not account.get("stale")
    )
    weekly_blocked = sum(
        1 for account in accounts if account.get("status") == "weekly_limited"
    ...
    *,
    ...
    for account in accounts:
        if (
            ...
            continue
        if account.get(window_key, {}).get("reported") is not True:
            continue
        label = str(account["label"])
        remaining = account.get(window_key, {}).get("remaining_percent")
        initial[label] = (
            float(remaining)
            if account.get("selectable_now")
            and isinstance(remaining, (int, float))
            and remaining > 0
            else 0.0
        )
        primary = account.get(window_key, {})
        ...
        while candidate <= horizon_end:
            eligible = (
                ...
                or (weekly_reset is not None and candidate >= weekly_reset)
            )
            events.append(
                {
                    "at": candidate,
...
def _window_label(key: str | None) -> str:
    if key == "five_hour":
        return "Five-hour"
    if key == "weekly":
        return "Weekly"
...
def _first_runout(
    ...
    *,
    ...
    for event in [
        *events,
        {"at": horizon_end, "account_label": None, "capacity_points": None},
    ]:
        ...
        if label is not None:
            ...
            expiries[label] = _following_expiry(
                label, events, after=event_at, horizon_end=horizon_end
            )
...
def _consume(
    ...
    for label in sorted(
        capacities,
        key=lambda item: (expiries.get(item, datetime.max.replace(tzinfo=UTC)), item),
    ):
        if demand <= 0:
            return
