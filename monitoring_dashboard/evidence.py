...
from monitoring_contracts.safe_evidence import safe_public_url
...
def _load_config(path: Path) -> JsonObject:
    parsed = cast("JsonInput", yaml.safe_load(path.read_text(encoding="utf-8")))
    return json_object(parsed)


def _require_dashboard_identity(request: Request, settings: RegistrySettings) -> None:
    ...
    for entry in object_list(config.get("domains")):
        if text_value(entry.get("domain")).lower() == domain:
            if entry.get("disabled") is True or entry.get("enabled") is False:
                raise HTTPException(status_code=409, detail="monitoring check is disabled")
            return entry
    raise HTTPException(status_code=404, detail="domain is not in the active monitoring inventory")
...
async def require_public_endpoint(url: str, *, inventory_domain: str) -> None:
    ...
    Raises:
        HTTPException: If the endpoint is not an allowlisted public target.
    """
    parsed = urlsplit(url)
...
def _request_spec(domain: str, request: Request) -> tuple[DomainCheckSpec, str]:
    application = cast("_RegistryApplication", request.app)
...
async def incident_evidence(domain: str, request: Request) -> JsonObject:
    ...
    Raises:
        HTTPException: If identity, inventory, or public-endpoint validation fails.
    """
    ...
    try:
        ...
        raise HTTPException(status_code=409, detail="monitoring evidence endpoint has an invalid port") from error
    except OSError as error:
        raise HTTPException(status_code=503, detail="monitoring evidence hostname did not resolve") from error
